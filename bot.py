...
from memory_manager import SemanticMemory # Import the new SemanticMemory system
...
# UserMemory class with token limit removed from semantic memory
class UserMemory:
    def __init__(self):
        self.users = {}
        self.memory_dir = "user_memories"
        self.semantic_memory = semantic_memory  # Add reference to semantic memory
        # Removed the max_tokens from here since we don't need it for semantic memory
    ...
    def add_message(self, user_id, role, content):
       ...
       # Remove oldest messages if token limit exceeded (kept from original)
        while self.users[user_id]["total_tokens"] > 1048576 and self.users[user_id]["messages"]:
    ...
    async def get_relevant_context(self, user_id, current_message=None, max_messages=10):
        """Get relevant conversation context for the user based on semantic similarity"""
        user_id = str(user_id)
        if user_id not in self.users:
            self.load_user_memory(user_id)
        ...
        # If no current message or very few messages, return basic context
        if not current_message or len(messages) < 3:
        ...
        # Use the semantic memory system for intelligent context retrieval
        try:
            # Create Gemini model for semantic analysis
            model = genai.GenerativeModel('gemini-2.0-flash-lite')
            
            # Get semantically relevant memories - using correct parameter name max_messages
            relevant_memories = await self.semantic_memory.find_relevant_memories(
                user_id, 
                current_message, 
                messages, 
                model,
                max_messages=max_messages  # This matches the parameter name in SemanticMemory
            )
            
            # Reconstruct rich context from relevant memories
            context = self.semantic_memory.reconstruct_context(
                relevant_memories,
                current_message
            )
            
            # Save the cache to ensure semantic memories persist
            self.semantic_memory.save_embedding_cache()
            
            # Return the contextually relevant conversation history
            return context
            
        except Exception as e:
            logging.error(f"Error retrieving semantic context: {e}")
    ...
    try:
        ...
        if update.message.text:
            ...
            try:
                ...
                # Get semantically relevant conversation history (NEW)
                MAX_RETRIES = 100
                retry_count = 0
                context_messages = []
                ...
                while retry_count < MAX_RETRIES:
                    try:
                        # Use the updated semantic context retrieval
                        context_messages = await user_memory.get_relevant_context(
                            user_id, 
                            current_message=message_text
                        )
                        
                        logger.info("Retrieved semantically relevant context")
                        ...
                        # Construct AI prompt with semantically relevant context
...
Previous conversation context (semantically selected for relevance):
{context_messages}
...
5. Remember information from the semantically selected context
